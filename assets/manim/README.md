...
Run `scripts/render-manim-lectures.sh` to regenerate all 45 assets listed in `manim/scene-manifest.json`. The script writes compact 480p, 24 fps MP4 and WebM versions plus a representative JPEG poster for each scene. Transitions use the slower study pace defined by `MANIM_LECTURE_PACE`.
